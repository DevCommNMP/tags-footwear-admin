...
const BasicProductDetails = () => {
  ...
  const handleSizeChange = (index, e) => {
    ...
    updatedSizes[index][name] = parseInt(value);
    setFormData({
      ...formData,
  ...
  return (
    <form onSubmit={handleSubmit} className="container mt-4">
      ...
      <div className="mb-3">
        <label htmlFor="sizesAvailable" className="form-label">Sizes Available:</label>
        {formData.sizesAvailable.map((size, index) => (
          <div key={index} className="d-flex mb-2">
            <select name="size" value={size.size} onChange={(e) => handleSizeChange(index, e)} className="form-select me-2">
            ...
            <input type="number" name="quantity" value={size.quantity} onChange={(e) => handleSizeChange(index, e)} className="form-control" />
          </div>
        ))}
        <button type="button" onClick={handleAddSize} className="btn btn-primary">Add Size</button>
        {errors.size && <span style={{ color: 'red' }}>{errors.size}</span>}
      </div>

      <div className="mb-3">
        <label htmlFor="colorsAvailable" className="form-label">Colors Available:</label>
        {['maroon', 'gold', 'peach', 'Brownx'].map((color, index) => (
          <div key={index} className="form-check form-check-inline">
            <input type="checkbox" id={color} name="colorsAvailable" value={color} onChange={handleColorChange} className="form-check-input" />
