...
export const fetchAllsubCategories = createAsyncThunk("products/categories", async (_, { rejectWithValue, getState }) => {
  try {
...
export const fetchAllFootrwearType = createAsyncThunk("products/footwearType", async (_, { rejectWithValue, getState }) => {
  try {
    // Check if the user is authenticated
    //   const localData = localStorage.getItem("userData"); // Assuming "auth" is the key where you store authentication data
